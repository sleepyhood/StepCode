...
def run_test():
    ...
    print("실제 세션 로그인 로직(Selenium) 검증 시작...", flush=True)
    try:
        # Selenium으로 교체된 메서드 호출 (Headed 모드로 테스트)
        session = app._login_selenium("http://edu.doingcoding.com", "test_user", "test_pass", False)
        if session:
