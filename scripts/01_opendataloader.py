...
class OpenDataLoaderGUI:
    def __init__(self, root):
        ...
        frame_format = tk.LabelFrame(
            root, text="3. 추출 포맷 및 옵션", padx=10, pady=10
        )
        ...
        tk.Checkbutton(
            frame_format, text="스캔 문서 모드", variable=self.var_scan_mode
    ...
    def run_conversion_task(self, inputs, out_dir, fmt, use_scan_mode):
        # 1. 스캔 모드인데 서버가 안 켜져 있으면 경고
        if use_scan_mode:
            try:
                # 5002번 포트가 응답하는지 확인
                ...
                self.root.after(
                    ...
                    lambda: messagebox.showerror(
                        "서버 오류",
                        "스캔 문서 모드를 쓰려면 터미널에서 'opendataloader-pdf-hybrid' 서버를 먼저 실행해야 합니다!",
                    ),
                )
                self.root.after(
                    0,
                    lambda: self.btn_run.config(
                        state=tk.NORMAL, text="일괄 추출 시작 (Batch Convert)"
                    ),
                )
                return
        try:
            ...
            opendataloader_pdf.convert(
                ...
                output_dir=out_dir,
                format=fmt,
                hybrid="docling-fast" if use_scan_mode else "off",
            )
