...
class MonitorSelectDialog(tk.Toplevel):
    def __init__(self, master: tk.Tk, monitor_previews: list[tuple[dict, Image.Image]]):
        ...
        ttk.Button(self, text="취소", command=self.destroy).pack(anchor="e", padx=12, pady=(0, 12))
...
class PdfCaptureApp:
    ...
    def _build_ui(self):
        ...
        self._labeled_spinbox(required_box, "시작 페이지 번호", self.start_page_var, 1, 999999, 1)
        self._labeled_spinbox(required_box, "캡쳐 장수", self.capture_count_var, 1, 999999, 1)
        self._labeled_spinbox(required_box, "시작 전 대기(초)", self.start_delay_var, 0, 30, 0.5)
        self._labeled_spinbox(required_box, "페이지 넘김 전 대기(초)", self.page_turn_delay_var, 0, 10, 0.1)
        self._labeled_spinbox(required_box, "우측키 후 대기(초)", self.post_key_delay_var, 0, 10, 0.1)
        ...
        self._labeled_spinbox(required_box, "다음 버튼 X 오프셋", self.next_click_x_offset_var, -200, 300, 1)
        self._labeled_spinbox(required_box, "다음 버튼 Y 오프셋", self.next_click_y_offset_var, -300, 300, 1)
        ...
        ttk.Checkbutton(optional_box, text="수동 선택 후 내부에서 자동 보정", variable=self.refine_manual_selection_var).pack(
            anchor="w", padx=12, pady=(10, 4)
        )
        ttk.Checkbutton(optional_box, text="캡쳐 전 브라우저 영역 클릭해서 포커스 확보", variable=self.click_to_focus_var).pack(
            anchor="w", padx=12, pady=4
        )
        self._labeled_spinbox(optional_box, "넘김 전 추가 최소(초)", self.page_turn_delay_extra_min_var, 0, 3, 0.05)
        self._labeled_spinbox(optional_box, "넘김 전 추가 최대(초)", self.page_turn_delay_extra_max_var, 0, 3, 0.05)
        self._labeled_spinbox(optional_box, "넘김 후 추가 최소(초)", self.post_key_delay_extra_min_var, 0, 3, 0.05)
        self._labeled_spinbox(optional_box, "넘김 후 추가 최대(초)", self.post_key_delay_extra_max_var, 0, 3, 0.05)
        self._labeled_spinbox(optional_box, "클릭 X 지터(px)", self.next_click_x_jitter_var, 0, 50, 1)
        self._labeled_spinbox(optional_box, "클릭 Y 지터(px)", self.next_click_y_jitter_var, 0, 50, 1)
        ttk.Separator(optional_box, orient="horizontal").pack(fill="x", padx=12, pady=8)
        ttk.Checkbutton(optional_box, text="가장자리 배경색 기준 자동 트리밍", variable=self.trim_edges_var).pack(
            anchor="w", padx=12, pady=4
        )
        ttk.Checkbutton(optional_box, text="상하 여백도 같이 트리밍", variable=self.trim_top_bottom_var).pack(
            anchor="w", padx=12, pady=4
        )
        self._labeled_spinbox(optional_box, "배경 허용 오차", self.bg_threshold_var, 1, 80, 1)
        self._labeled_spinbox(optional_box, "안전 여백(px)", self.edge_margin_var, 0, 30, 1)
    ...
    def select_region(self):
        ...
        def handle_region(region: CaptureRegion):
            ...
            self._apply_capture_region(region, prefix=None, status_message="캡쳐 영역이 설정되었습니다.")
    ...
    def start_capture(self):
        ...
        config = CaptureJobConfig(
            ...
            page_turn_delay_extra_min=max(0.0, float(self.page_turn_delay_extra_min_var.get())),
            page_turn_delay_extra_max=max(0.0, float(self.page_turn_delay_extra_max_var.get())),
            post_key_delay_extra_min=max(0.0, float(self.post_key_delay_extra_min_var.get())),
            post_key_delay_extra_max=max(0.0, float(self.post_key_delay_extra_max_var.get())),
    ...
    def _click_next_button(self, x_offset: int, y_offset: int, x_jitter: int = 0, y_jitter: int = 0):
        if not self.capture_region:
            return
        ...
        y = self.capture_region.top + self.capture_region.height // 2 + y_offset + jitter_y
        pyautogui.click(x, y)

    def _apply_delay_range(self, base_delay: float, extra_min: float, extra_max: float) -> float:
    ...
    def _choose_monitor(self) -> dict | None:
        ...
        monitor_previews = [(monitor, self._grab_monitor_image(monitor)) for monitor in monitors]
    ...
    def _detect_pdf_region_with_debug(self, image: Image.Image) -> tuple[CaptureRegion | None, DetectionDebugInfo]:
        ...
        inner_candidates = self._find_inner_page_candidates(gray, bright_candidates, screen_w, screen_h)
    ...
    def _refine_selected_region(self, region: CaptureRegion):
        try:
            self._set_status("선택 영역 내부에서 PDF 페이지 경계를 자동 보정하는 중입니다...")
            time.sleep(0.15)
    ...
    def _apply_capture_region(self, region: CaptureRegion, prefix: str | None, status_message: str):
    ...
    def _save_detection_debug_image(
        ...
        for index, candidate in enumerate(ranked_candidates[:12], start=1):
            ...
            if candidate.accepted:
                ...
                color = rejected_bright_color if candidate.source == "bright" else rejected_edge_color
    ...
    def _find_bright_page_candidates(self, gray: np.ndarray) -> list[tuple[int, int, int, int]]:
    ...
    def _find_edge_based_candidates(self, gray: np.ndarray) -> list[tuple[int, int, int, int]]:
        blur = cv2.GaussianBlur(gray, (5, 5), 0)
    ...
    def _estimate_inner_content_rect(self, gray: np.ndarray) -> tuple[int, int, int, int] | None:
