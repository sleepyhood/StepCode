...
### 지문
아래 코드 조각이 **ECS libraries**(예: `Unity.Entities`)를 사용하는지 평가하시오.  
...
2. 코드 2는 `using Unity.Entities;`를 포함해 **Entities(ECS) 라이브러리를 참조**한다.
3. 코드 3은 `ComponentSystem`을 상속하므로 **전형적인 ECS 코드**다.

#### 문항별 해설(부분 점수 포인트)

* **[코드 1] 거짓**
  `Unity.Entities`를 참조하지 않고, ECS 타입(`ComponentSystem`, `IComponentData` 등)도 없다. → ECS 사용 X

* **[코드 2] 참**
  `Unity.Entities` 네임스페이스를 가져오므로 **ECS 라이브러리를 사용(참조)**하는 코드 조각으로 본다.
  *(헷갈리면: 이 코드 자체가 “정석 ECS 컴포넌트” 형태는 아니지만, 문제는 ‘ECS 라이브러리를 쓰는지’ 판단하는 것이므로 `Unity.Entities` 포함 여부가 핵심 힌트다.)*

* **[코드 3] 참**
  `ComponentSystem`은 Entities(ECS) 쪽 시스템 베이스 클래스다. → ECS 사용 O

#### 오답 포인트(헷갈리는 지점)

* “MonoBehaviour면 무조건 ECS 아님”으로 단정하면 **코드 2**에서 틀릴 수 있다.
  이 문항은 “ECS 스타일로 작성했나?”가 아니라 **ECS 라이브러리(Entities)를 쓰나?**를 묻는다.

#### 최종 정리

* 코드 1: 거짓 / 코드 2: 참 / 코드 3: 참
...
각 문장이 **참인지 거짓인지** 선택하세요. *(부분 크레딧이 있을 수 있습니다.)*
...
코드가 수행하는 기능을 **정확하게 설명하는 주석 2개**를 선택하세요. *(2개 선택)*
...
각 `if (Input.[빈칸])`에 들어갈 **올바른 입력 메서드**를 드롭다운에서 선택해 코드를 완성하세요. *(부분 점수 있음)*
