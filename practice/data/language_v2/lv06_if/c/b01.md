---
id: "c_if_b1"
title: "C 조건문 기초 1회차"
categoryId: "c_if"
availableLanguages: ["c"]
---

# C 조건문 기초 1회차

### Q1. MCQ 1. Pass / Fail 프로그램

다음 설명에 맞는 C 코드를 고르세요.

- 정수 score를 하나 입력받는다.
- score가 60 이상이면 "Pass"를 출력하고,
  그렇지 않으면 "Fail"을 출력한다.

- **A**: char score; scanf(" %c", &score); if (score >= 60) {     printf("Pass\n"); } else {     printf("Fail\n"); }
- **B**: int score; scanf("%d", &score); if (score > 60) {     printf("Pass\n"); } else {     printf("Fail\n"); }
- **C**: int score; scanf("%d", &score); if (score >= 60) {     printf("Pass\n"); } else {     printf("Fail\n"); }
- **D**: int score; scanf("%d", &score); if (score == 60) {     printf("Pass\n"); } else {     printf("Fail\n"); }

---
<!-- ANSWER_START -->
### [답안 및 해설]
- **정답 인덱스:** 2
<!-- ANSWER_END -->

### Q2. MCQ 2. 짝수/홀수 출력 결과

아래 프로그램에서 입력이 3일 때, 출력으로 알맞은 것을 고르세요.

```c
int n;
scanf("%d", &n);
if (n % 2 == 0) {
    printf("Even\n");
}
else {
    printf("Odd\n");
}
printf("Done\n");
```

- **A**: Even Done
- **B**: Odd Done
- **C**: 3 Odd Done
- **D**: Odd 3 Done

---
<!-- ANSWER_START -->
### [답안 및 해설]
- **정답 인덱스:** 1
<!-- ANSWER_END -->

### Q3. MCQ 3. 범위 검사

다음 프로그램은 1 이상 10 이하이면 "InRange"를 출력합니다.
"InRange"가 출력되는 입력을 모두 고른 것은?

```c
int n;
scanf("%d", &n);
if (1 <= n && n <= 10) {
    printf("InRange\n");
}
```

- **A**: 0, 1, 10
- **B**: 1, 5, 10
- **C**: 1, 10, 11
- **D**: 0, 5, 11

---
<!-- ANSWER_START -->
### [답안 및 해설]
- **정답 인덱스:** 1
<!-- ANSWER_END -->

### Q4. MCQ 4. 항상 Minus가 나오는 이유

다음 프로그램은 음수일 때만 "Minus"를 출력하려고 했지만,
입력 값과 상관없이 항상 "Minus"가 출력됩니다.
가장 중요한 원인은 무엇인가요?

```c
int n;
scanf("%d", &n);
if (n < 0) {
    n = n;
}
printf("Minus\n");
```

- **A**: 변수 n의 자료형이 잘못되었다.
- **B**: 조건식이 n <= 0 이어야 한다.
- **C**: printf문이 if 블록 안으로 작성되어야 한다.
- **D**: scanf()에서 정수로 입력받지 않았다.;

---
<!-- ANSWER_START -->
### [답안 및 해설]
- **정답 인덱스:** 2
<!-- ANSWER_END -->

### Q5. MCQ 5. 학점 조건식

다음은 점수에 따라 학점을 출력하려는 프로그램입니다.
- 90 이상이면 "A"
- 80 이상 90 미만이면 "B"
- 나머지는 "C"를 출력하려고 합니다.
올바른 코드 한 개를 고르세요.

- **A**: int score; scanf("%d", &score); if (score >= 90) {     printf("A\n"); } if (score >= 80) {     printf("B\n"); } else {     printf("C\n"); }
- **B**: int score; scanf("%d", &score); if (score >= 80) {     printf("B\n"); } else if (score >= 90) {     printf("A\n"); } else {     printf("C\n"); }
- **C**: int score; scanf("%d", &score); if (score >= 90) {     printf("A\n"); } else if (score >= 80) {     printf("B\n"); } else {     printf("C\n"); }
- **D**: int score; scanf("%d", &score); if (score > 90) {     printf("A\n"); } else if (score > 80) {     printf("B\n"); } else {     printf("C\n"); }

---
<!-- ANSWER_START -->
### [답안 및 해설]
- **정답 인덱스:** 2
<!-- ANSWER_END -->

### Q6. Trace 1. 평균으로 Pass/Fail 분기

두 점수의 평균에 따라 분기되는 결과를 표에 채우세요.

입력 케이스 예시:
- case1: s1=80, s2=70
- case2: s1=50, s2=40
- case3: s1=60, s2=60

```c
int s1, s2;
scanf("%d %d", &s1, &s2);
int avg = (s1 + s2) / 2;
if (avg >= 60) {
    printf("PASS\n");
}
else {
    printf("FAIL\n");
}
```

---
<!-- ANSWER_START -->
### [답안 및 해설]
- **Trace 정답 표:** [["80", "70", "75", "true", "PASS"], ["50", "40", "45", "false", "FAIL"], ["60", "60", "60", "true", "PASS"]]
<!-- ANSWER_END -->

### Q7. Short 1. 범위 + if-else 결과

아래 프로그램에서 입력이 7일 때, 출력되는 내용을 그대로 쓰세요.
(줄바꿈 없이 한 줄입니다.)

```c
int n;
scanf("%d", &n);
if (5 <= n && n <= 10) {
    printf("OK\n");
}
else {
    printf("NG\n");
}
```

---
<!-- ANSWER_START -->
### [답안 및 해설]
- **정답:** `OK`
<!-- ANSWER_END -->

### Q8. Short 2. 어떤 값을 넣으면 참이 될까?

다음 조건식이 참(true)이 되도록 n의 값을 하나만 쓰세요.
(여러 개 중 아무거나 하나 맞으면 정답입니다.)

```c
1 <= n && n <= 5;
```

---
<!-- ANSWER_START -->
### [답안 및 해설]
- **복수 정답 허용:** ["1", "2", "3", "4", "5"]
<!-- ANSWER_END -->

### Q9. Code 1. 1 이상 10 이하 조건식 작성

변수 n이 1 이상 10 이하일 때만 if 안이 실행되도록, 괄호 안의 조건식을 작성하세요.

if 뒤에 들어갈 조건식을 한 줄로 쓰세요.
(범위 비교: n >= 1 && n <= 10 형태를 사용하세요.)

```c
int n;
scanf("%d", &n);
if (/* 여기에 조건식을 작성하세요 */) {
    printf("InRange\n");
}
```

---
<!-- ANSWER_START -->
### [답안 및 해설]
- **모범 코드:**
```c
n >= 1 && n <= 10
```
<!-- ANSWER_END -->

### Q10. Code 2. 두 조건을 모두 만족해야 Pass

정수 score와 absent를 입력받았을 때,
- score가 60 이상이고
- absent(결석 횟수)가 5 미만일 때만 "Pass"를 출력하고,
그렇지 않으면 "Fail"을 출력하는 조건식을 작성하세요.

if 뒤 괄호 안에 들어갈 조건식만 한 줄로 쓰면 됩니다.

```c
int score;
scanf("%d", &score);
int absent;
scanf("%d", &absent);
if (/* 여기에 조건식을 작성하세요 */) {
    printf("Pass\n");
}
else {
    printf("Fail\n");
}
```

---
<!-- ANSWER_START -->
### [답안 및 해설]
- **모범 코드:**
```c
score >= 60 && absent < 5
```
<!-- ANSWER_END -->
