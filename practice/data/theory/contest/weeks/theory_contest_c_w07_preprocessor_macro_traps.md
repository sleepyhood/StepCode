...
## 학습 목표
- 매크로를 텍스트 치환 관점에서 전개하고 계산합니다.
- 매크로 괄호 유무에 따른 우선순위 오류와 부작용을 판별합니다.
- 조건부 컴파일에서 실제 활성 코드 블록을 정확히 읽습니다.
- 개념 수 운영 메모: W07은 치환 규칙, 괄호 안정성, 조건부 컴파일의 3축을 묶어 학습하는 주차
...
- 핵심 내용:
  - 매크로는 함수 호출이 아니라 문자열 치환으로 먼저 바뀝니다.
  - 치환이 끝난 식을 C 연산 우선순위로 계산합니다.
  - 괄호가 없으면 의도와 다른 식으로 계산될 수 있습니다.
- 판별 규칙:
  - 호출식을 본 뒤 반드시 치환식을 먼저 써보고 계산한다.
...
- 핵심 내용:
  - 안전한 매크로는 보통 `#define F(x) ((x) * (x))`처럼 괄호를 충분히 사용합니다.
  - `#define f(x) x+1`처럼 괄호가 없으면 곱셈과 함께 쓸 때 결과가 바뀔 수 있습니다.
  - 같은 호출식도 매크로 정의 방식에 따라 값이 달라질 수 있습니다.
- 판별 규칙:
  - 괄호 없는 버전과 괄호 있는 버전을 나란히 비교해 계산한다.
...
- 핵심 내용:
  - 거짓 조건 블록은 컴파일 대상에서 빠집니다.
  - 매크로가 \"정의되었는지\"와 \"값이 참인지\"는 다른 기준입니다.
  - 먼저 활성 블록을 정한 뒤에 실행 흐름을 보는 순서가 안전합니다.
- 판별 규칙:
  - `#ifdef`는 정의 여부, `#if`는 식의 참/거짓으로 구분해 읽는다.
