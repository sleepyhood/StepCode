...
## 학습 목표
- 포인터 역참조와 주소 전달 규칙으로 실제 접근 대상을 추적합니다.
- malloc, calloc, realloc, free 생명주기와 상태 변화를 구분합니다.
- 구조체 필드(front, rear) 갱신 시점과 출력 구간을 계산합니다.
- 개념 수 운영 메모: W10은 포인터 접근, 동적 메모리 생명주기, 구조체 인덱스 추적의 3축을 묶어 학습하는 주차
...
- 핵심 내용:
  - 포인터 변수에는 값 자체가 아니라 주소가 저장됩니다.
  - `*p`는 주소가 가리키는 실제 데이터이며, `p[i]`는 `*(p+i)`와 같은 접근입니다.
  - 주소를 함수에 넘기면 함수 안에서 원본 데이터가 직접 바뀔 수 있습니다.
- 판별 규칙:
  - 포인터 문항은 `주소인지 값인지`를 먼저 구분한 뒤 인덱스 접근을 해석한다.
...
- 핵심 내용:
  - `malloc`은 메모리를 만들기만 하고 초기값은 보장하지 않습니다.
  - `calloc`은 메모리를 만들면서 0으로 초기화합니다.
  - `realloc`은 크기 조정 과정에서 주소가 바뀔 수 있고, `free`는 사용이 끝난 메모리를 해제합니다.
- 판별 규칙:
  - 동적 메모리 문제는 `할당 -> 사용 -> 재할당 -> 해제` 순서로 상태를 확인한다.
...
- 핵심 내용:
  - `front`, `rear`는 현재 유효 데이터 구간의 시작과 끝을 나타냅니다.
  - 전위/후위 증감은 인덱스를 바꾸는 시점이 달라 저장 위치를 바꿉니다.
  - 최종 출력은 `front`부터 `rear`까지의 순회 구간으로 결정됩니다.
- 판별 규칙:
  - 구조체 추적은 `인덱스 변화 시점`을 먼저 적고 저장/출력 순서를 계산한다.
