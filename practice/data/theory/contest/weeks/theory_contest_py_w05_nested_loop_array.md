...
### 개념 1) 이중합 분해 (`Σi Σj`) 
- 개념 정의: 2중 반복에서 곱 형태 누적을 곱셈 구조로 축약하는 유형입니다.
- 판별 규칙: `Σi Σj (Ai*Bj)` 형태는 `(ΣAi) * (ΣBj)`로 분해 가능하다.
- 추적 절차: 각 배열 합 계산 -> 곱셈으로 이중합 계산 -> 후처리 연산 적용한다.
- 오답 포인트: 내부 루프를 모두 전개해 계산하다가 합산 실수가 자주 발생합니다.
...
- 개념 정의: 3중 반복 누적식에서 항을 분리해 반복 배수를 계산하는 유형입니다.
- 판별 규칙: `a[i] + b[j]*c[k]`는 `a`항과 `b*c`항을 분리해 반복 배수로 계산할 수 있다.
- 추적 절차: 각 차원 반복 횟수 계산 -> 항별 누적 배수 산출 -> 최종 합 결합한다.
- 오답 포인트: 모든 항이 같은 배수로 더해진다고 가정하면 오답이 됩니다.
...
- 개념 정의: 다중 반복으로 후보 값을 비교하며 최대/최소를 갱신하는 유형입니다.
- 판별 규칙: `if candidate > best: best = candidate` 패턴은 초기값과 비교 방향이 핵심이다.
- 추적 절차: 초기값 설정 -> 후보 생성 -> 조건 비교 -> 갱신 여부 기록한다.
- 오답 포인트: 초기값을 너무 크게/작게 두면 갱신이 일어나지 않아 결과가 깨집니다.
...
- 개념 정의: 중첩 반복에서 특정 조건 만족 시 탐색을 건너뛰거나 조기 종료하는 유형입니다.
- 판별 규칙: `continue`는 현재 반복만 건너뛰고, `break`는 현재 루프 1단계만 종료한다.
- 추적 절차: 루프 깊이를 번호로 표시 -> `continue/break` 적용 범위를 기록 -> 플래그 전파 흐름 확인한다.
- 오답 포인트: `break`가 모든 루프를 한 번에 끝낸다고 오해하면 실행 순서가 뒤틀립니다.
