# Python 경시대회 W05 2중/3중 반복/배열 탐색

## 메타
- week: W05
- slug: nested_loop_array
- audience: common, elementary, middle, high

## 학습 목표
- 2중/3중 반복에서 총 연산량과 누적 구조를 분리해 해석합니다.
- 배열/리스트 기반 탐색에서 최댓값 갱신 규칙을 안정적으로 적용합니다.
- `continue`, `break`, 플래그 변수로 중첩 루프를 제어하는 흐름을 설명합니다.
- 개념 수 운영 메모: W05는 이중합 분해, 3중 항 분리, 최댓값 갱신, 루프 제어의 4축으로 운영합니다.

## 대상 난이도
- 공통: 초등, 중등, 고등
- 분기: ELEMENTARY, MIDDLE, HIGH

## 공통 이론 (COMMON)
<!-- audience:common -->
### 개념 1) 이중합 분해 (`Σi Σj`) 
- 개념 정의: 2중 반복의 곱 형태 누적을 수식 분해로 빠르게 계산하는 개념입니다.
- 핵심 내용:
  - `Σi Σj (Ai*Bj)`는 `(ΣAi) * (ΣBj)`로 분해할 수 있습니다.
  - 배열 합을 먼저 구하면 중첩 반복 전개 없이 계산이 가능합니다.
  - 식 모양이 다르면(예: `Ai + Bj`) 분해 방식도 달라집니다.
- 오답 포인트: 내부 루프를 끝까지 전개하다가 합산 실수나 항 분해 오류가 발생합니다.

![이중합 분해식 블록 다이어그램](./data/theory/images/contest_w05_double_sum_decomposition.svg)

예시 (기본):
```python
a = [1, 2, 3]
b = [4, 5]
s = 0
for i in range(len(a)):
    for j in range(len(b)):
        s += a[i] * b[j]
print(s)
```
```io
input:
(없음)
output:
54
```

예시 (변형):
```python
a = [0, 1, 2, 3]
b = [1, -1]
s = 0
for i in range(len(a)):
    for j in range(len(b)):
        s += a[i] * b[j]
print(s)
```
```io
input:
(없음)
output:
0
```

예시 (함정):
```python
a = [2, 4]
b = [3, 6]
s = 0
for i in range(2):
    for j in range(2):
        s += a[i] + b[j]
print(s)
```
```io
input:
(없음)
output:
30
```

### 개념 2) 3중 반복에서 항 분리 계산
- 개념 정의: 3중 반복 누적식을 항별로 나눠 반복 배수로 계산하는 개념입니다.
- 핵심 내용:
  - `a[i] + b[j]*c[k]`는 항을 분리해 각 항의 반복 배수를 따로 계산합니다.
  - 차원별 반복 횟수를 먼저 고정하면 항별 총합 계산이 쉬워집니다.
  - 분기(`if`)가 있으면 항별 부호/배수가 달라질 수 있습니다.
- 오답 포인트: 모든 항이 같은 배수로 누적된다고 가정하면 최종 합이 틀립니다.

예시 (기본):
```python
a = [1, 2]
b = [0, 3]
c = [1, 2]
t = 0
for i in range(2):
    for j in range(2):
        for k in range(2):
            t += a[i] + b[j] * c[k]
print(t)
```
```io
input:
(없음)
output:
36
```

예시 (변형):
```python
x = [2, 4, 6]
y = [1, 3]
z = [0, 5]
acc = 0
for i in range(3):
    for j in range(2):
        for k in range(2):
            acc += x[i] - y[j] + z[k]
print(acc)
```
```io
input:
(없음)
output:
60
```

예시 (함정):
```python
a = [1, 2]
b = [10, 20]
c = [0, 1]
s = 0
for i in range(2):
    for j in range(2):
        for k in range(2):
            if k == 0:
                s += a[i] + b[j]
            else:
                s -= a[i] + b[j]
print(s)
```
```io
input:
(없음)
output:
0
```

### 개념 3) 배열 탐색 최댓값 갱신 패턴
- 개념 정의: 중첩 반복에서 후보값을 비교해 최댓값/최솟값을 갱신하는 개념입니다.
- 핵심 내용:
  - `if candidate > best: best = candidate` 패턴은 초기값 선택이 핵심입니다.
  - 후보 생성식과 비교 방향(`>`/`<`)을 함께 확인해야 합니다.
  - 갱신이 한 번도 일어나지 않는 경우도 검증해야 합니다.
- 오답 포인트: 초기값이 범위에 맞지 않으면 갱신이 막혀 결과가 깨집니다.

예시 (기본):
```python
a = [8, 3, 10]
b = [1, 7]
best = 0
for x in a:
    for y in b:
        if x + y > best:
            best = x + y
print(best)
```
```io
input:
(없음)
output:
17
```

예시 (변형):
```python
a = [-3, -1, -7]
b = [2, 5]
best = -10**9
for x in a:
    for y in b:
        if x + y > best:
            best = x + y
print(best)
```
```io
input:
(없음)
output:
4
```

예시 (함정):
```python
a = [5, 1]
b = [2, 3]
best = 100
for x in a:
    for y in b:
        if x + y > best:
            best = x + y
print(best)
```
```io
input:
(없음)
output:
100
```

### 개념 4) 중첩 루프 제어 (`continue`/`break`/플래그)
- 개념 정의: 중첩 반복에서 `continue`, `break`, 플래그로 실행 범위를 제어하는 개념입니다.
- 핵심 내용:
  - `continue`는 현재 루프의 이번 회차만 건너뜁니다.
  - `break`는 현재 루프 한 단계만 종료합니다.
  - 다중 루프 조기 종료는 플래그 전파나 함수 반환으로 처리합니다.
- 오답 포인트: `break`가 모든 루프를 즉시 종료한다고 오해하면 실행 흐름을 잘못 읽습니다.

| 제어문 | 즉시 영향 범위 | 바깥 루프 영향 | 대표 사용 목적 | 주의점 |
| --- | --- | --- | --- | --- |
| `continue` | 현재 루프의 "이번 회차"만 건너뜀 | 없음 | 특정 조건 스킵 | 아래 코드 미실행 |
| `break` | 현재 루프 1단계 종료 | 직접 영향 없음 | 조기 탈출 | 중첩일 때 한 단계만 종료 |
| `flag + break` | 현재 루프 종료 후 플래그 확인 | 있음(간접) | 다중 루프 탈출 | 플래그 검사 위치 필수 |

![continue/break 적용 범위 루프 깊이 트리](./data/theory/images/contest_w05_loop_control_depth_tree.svg)

예시 (기본):
```python
for i in range(3):
    for j in range(3):
        if j == 1:
            continue
        print(i, j)
```
```io
input:
(없음)
output:
0 0
0 2
1 0
1 2
2 0
2 2
```

예시 (변형):
```python
flag = 0
for i in range(1, 5):
    for j in range(1, 5):
        if i * j == 6:
            print(i, j)
            flag = 1
            break
    if flag:
        break
```
```io
input:
(없음)
output:
2 3
```

예시 (함정):
```python
for i in range(2):
    for j in range(3):
        if j == 1:
            break
        print(i, j)
    print("-")
```
```io
input:
(없음)
output:
0 0
-
1 0
-
```

### 공통 미니 체크 (필수 제출)
> 제한 시간 5분. 정답만 쓰지 말고, 각 문항에 근거 한 줄을 함께 작성합니다.

문항:
1. `Σi Σj (Ai*Bj)`를 빠르게 계산하는 변환식을 쓰시오.
2. 3중 반복 식에서 항 분리를 할 때 먼저 계산해야 하는 값 2가지는 무엇인가요?
3. 중첩 루프에서 `break`가 영향을 주는 범위를 어떻게 판단하나요?

답안 작성:
1. 정답: [ ] / 근거: [ ]
2. 정답: [ ] / 근거: [ ]
3. 정답: [ ] / 근거: [ ]
## 초등 트랙 (ELEMENTARY)
<!-- audience:elementary -->
### 초등 포인트
- 2중 반복 카운트

### 초등 이론 보강
- 반복 횟수와 누적 변수를 표로 동시에 적는 연습을 합니다.
- 최댓값 갱신은 "초기값 -> 후보 비교 -> 갱신" 3단계로 고정합니다.

초등 규칙 카드:
1. 반복 1회마다 누적 변수 변화를 기록한다.
2. `best` 초기값은 문제 범위에 맞게 둔다.
3. `continue`는 현재 반복만 건너뛴다.

### 초등 연계 실습 (핸즈온)
실습 목표:
- 핵심 규칙을 직접 계산/표기로 확인합니다.

실습 문제 코드:
```python
# W05 starter (초등): 2중 반복 카운트

def solve():
    cnt = 0
    for i in range(3):
        for j in range(2):
            cnt += 1
    print(cnt)

if __name__ == "__main__":
    solve()
```

과제:
1. 2중 반복에서 연산문이 총 몇 번 실행되는지 계산하시오.
2. 후보값 갱신 문제에서 초기값을 바꾸면 결과가 어떻게 달라지는지 설명하시오.

체크포인트:
1. 과제별로 코드를 직접 실행해 결과를 확인했는가?
2. 정답과 함께 근거(판별 규칙/계산 과정)를 기록했는가?
3. 오답 가능 지점을 한 줄로 점검했는가?
### 초등 오답 패턴
- 비교 연산은 맞게 쓰지만 초기값이 잘못되어 결과가 틀립니다.

## 중등 트랙 (MIDDLE)
<!-- audience:middle -->
### 중등 포인트
- 2중 반복 + 조건 결합

### 중등 이론 보강
- 조건별 집합을 나눠 부분합으로 계산한 뒤 결합합니다.
- `if` 두 개와 `if-else`의 실행 차이를 분리해서 해석합니다.

중등 판별 루틴:
1. 반복 범위를 확정한다.
2. 조건 통과 케이스를 분리한다.
3. 누적식을 항별로 계산해 결합한다.

### 중등 연계 실습 (핸즈온)
실습 목표:
- 판별 절차를 적용해 근거 중심으로 답안을 작성합니다.

실습 문제 코드:
```python
# W05 starter (중등): 배열 탐색 + 조건 누적

def solve():
    arr = [4, 7, 1, 9, 3]
    mx = arr[0]
    idx = 0
    for i, v in enumerate(arr):
        if v > mx:
            mx = v
            idx = i
    print(mx, idx)

if __name__ == "__main__":
    solve()
```

과제:
1. 조건이 포함된 3중 반복 코드를 항 분리 방식으로 풀이하시오.
2. `continue`가 있는 코드에서 실제 실행 줄만 추려 쓰시오.

체크포인트:
1. 과제별로 코드를 직접 실행해 결과를 확인했는가?
2. 정답과 함께 근거(판별 규칙/계산 과정)를 기록했는가?
3. 오답 가능 지점을 한 줄로 점검했는가?
### 중등 오답 패턴
- 조건 분기를 모두 같은 누적식으로 처리해 부호/배수가 틀립니다.

## 고등 트랙 (HIGH)
<!-- audience:high -->
### 고등 포인트
- 2중 반복 + 형변환 결합

### 고등 이론 보강
- 문자열 결합/정수 변환이 들어간 반복 탐색은 "생성 -> 필터 -> 검증" 순서로 해석합니다.
- 조기 종료(`break`)와 탐색 완전성의 균형을 판단하는 훈련이 필요합니다.

고등 판별 프레임:
1. 후보 생성 규칙을 정의한다.
2. 배제 조건(`continue`)을 먼저 적용한다.
3. 검증식 통과 시 종료 범위를 확인한다.

### 고등 연계 실습 (핸즈온)
실습 목표:
- 판별 근거를 먼저 세우고 결과를 검증합니다.

실습 문제 코드:
```python
# W05 starter (고등): 중첩 루프 제어(break/flag)

def solve():
    grid = [[1, 0, 0], [0, 1, 0], [0, 0, 1]]
    found = False
    for r in range(3):
        for c in range(3):
            if grid[r][c] == 1 and r == c:
                found = True
                break
        if found:
            break
    print(found)

if __name__ == "__main__":
    solve()
```

과제:
1. 4중 반복 탐색에서 가지치기 조건을 2개 이상 설계하시오.
2. 플래그 변수를 사용하지 않고 조기 종료하는 방법을 제시하시오.

체크포인트:
1. 과제별로 코드를 직접 실행해 결과를 확인했는가?
2. 정답과 함께 근거(판별 규칙/계산 과정)를 기록했는가?
3. 오답 가능 지점을 한 줄로 점검했는가?
### 고등 오답 패턴
- 정답 후보는 찾았지만 종료 범위를 잘못 처리해 출력이 어긋납니다.

## 적용 유형
- 이중합/삼중합 계산
- 배열/리스트 최댓값 탐색
- 조건 분기 포함 중첩 반복 추적
- `continue`/`break` 조기 종료 해석

## {view:teacher} 과제
- 초등: 6문항
- 중등: 8문항
- 고등: 10~12문항

## {view:teacher} 평가
- 항목: 미니모의 1
- 오답코드: 인덱스, 종료조건, 방문처리, 형변환, 연산자 우선순위

평가 체크:
1. 항 분리/부분합으로 계산 근거를 제시했는가
2. 최댓값 갱신 규칙을 초기값부터 설명했는가
3. 조기 종료 범위를 루프 깊이 기준으로 설명했는가

## {view:teacher} 교사 메모
- 웹 렌더 규칙: COMMON + 선택 학년 섹션만 노출합니다.
- 수업 후 오답코드를 기준으로 다음 주차 보강 포인트를 기록합니다.
