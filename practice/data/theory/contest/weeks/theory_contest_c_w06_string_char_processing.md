...
## 학습 목표
- 널 문자와 strlen 규칙으로 문자열 길이를 정확히 계산합니다.
- ASCII 기반 문자 변환과 비교, 빈도 인덱싱을 적용합니다.
- 문자열 순회와 치환 패턴, string.h 함수 역할을 구분합니다.
- 개념 수 운영 메모: W06은 문자열 길이, 문자 코드, 문자열 순회의 3축을 묶어 학습하는 주차
...
- 핵심 내용:
  - `strlen`은 첫 `\0` 전까지의 문자 개수만 셉니다.
  - `"...\0..."` 형태에서는 `\0` 뒤 문자는 길이에 포함되지 않습니다.
  - `\n`은 길이 0이 아니라 문자 1개로 계산됩니다.
- 판별 규칙:
  - 문자열 길이 문제는 먼저 `\0` 위치를 찾고 그 앞 문자 수를 센다.
...
- 핵심 내용:
  - 영문 대문자와 소문자 코드 차이는 32입니다.
  - 문자 비교(`>`, `<`)는 글자 모양이 아니라 코드값 크기 비교입니다.
  - 알파벳 빈도 배열은 `cnt[ch - 'a']`처럼 인덱스로 접근합니다.
- 판별 규칙:
  - 문자 문제는 글자 자체가 아니라 코드값으로 바꿔 판단한다.
...
- 핵심 내용:
  - 문자열 순회는 보통 `str[i] != '\0'`를 종료 조건으로 사용합니다.
  - 연속 중복 제거는 `str[i]`와 `str[i+1]` 비교로 구현할 수 있습니다.
  - `strcpy`, `strcmp`, `strchr`, `strcat`은 역할이 서로 다릅니다.
- 판별 규칙:
  - 함수 이름을 보면 먼저 \"복사/비교/탐색/결합\" 중 무엇인지 분류한다.
