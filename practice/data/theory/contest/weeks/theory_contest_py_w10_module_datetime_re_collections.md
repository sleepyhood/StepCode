...
### 개념 1) 모듈-용도 매핑 규칙
- 한 줄 요약: 모듈 이름과 대표 기능을 연결해 맞는 짝/틀린 짝을 고르는 유형입니다.
- 암기 규칙: 모듈의 핵심 책임을 1문장으로 고정해 판단한다.
- 풀이 3단계: 모듈명 확인 -> 대표 기능 키워드 대입 -> 보기 문장과 일치 여부 판정 순으로 풀이합니다.
- 함정 포인트: 함수 이름 하나만 보고 모듈 전체 용도를 오해하면 오답이 됩니다.
...
### 개념 2) `os` 함수 존재 여부 판별
- 한 줄 요약: `os` 모듈에 실제로 정의된 함수인지 이름을 보고 판별하는 유형입니다.
- 암기 규칙: 존재 여부는 문서/자동완성 또는 `hasattr(os, name)`로 확인한다.
- 풀이 3단계: 함수 이름 정확히 읽기 -> 철자 오탈자 점검 -> `os` 네임스페이스에 존재하는지 검증 순으로 판단합니다.
- 함정 포인트: 비슷한 철자(`getpid` vs `seekid`)를 혼동하면 오답이 됩니다.
...
- 한 줄 요약: 정규식이 문자열 전체와 일치할 때만 매치 객체를 반환하는 유형입니다.
- 암기 규칙: `fullmatch`는 부분 일치가 아니라 전체 일치를 요구한다.
- 풀이 3단계: 패턴 해석 -> 대상 문자열 전체 비교 -> 매치/None 판정 -> 줄 번호 매핑 순으로 풀이합니다.
- 함정 포인트: `search`처럼 부분 매칭으로 해석하면 결과를 반대로 고르게 됩니다.
...
- 한 줄 요약: 날짜/시간 객체를 포맷 문자열 코드로 변환해 출력하는 유형입니다.
- 암기 규칙: `%Y` 연도, `%B` 월 이름, `%M` 분, `%a` 요일 축약, `%Z` 시간대, `%p` AM/PM이다.
- 풀이 3단계: 포맷 코드 분해 -> 각 코드 의미 대입 -> 예시 문자열과 일치 여부 판정 순으로 풀이합니다.
- 함정 포인트: `%Z`와 `%p`를 혼동하면 오답이 됩니다.
...
- 한 줄 요약: `collections`가 제공하는 특수 컨테이너가 어떤 내장 컨테이너 계열 확장인지 판별하는 유형입니다.
- 암기 규칙: 대표 대응은 `deque/list`, `defaultdict/dict`, `namedtuple/tuple`, `Counter/dict`다.
- 풀이 3단계: 컬렉션 이름 확인 -> 기반 컨테이너 계열 매핑 -> 보기와 일치 여부 판정 순으로 풀이합니다.
- 함정 포인트: 문자열(`str`)을 `collections` 대체 컨테이너로 착각하면 오답이 됩니다.
