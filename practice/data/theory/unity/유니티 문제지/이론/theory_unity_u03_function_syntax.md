...
null을 반환하는 것이 아니다. void는 그냥 반환값이 없다.

### 2) 유니티의 특별한 함수
- 일반적인 함수들은 내가 직접 호출을 해줘야 실행이 된다.  
그러나, 유니티에서 사용하는 특별한 함수들은 내가 직접 호출하지 않아도 유니티가 상황에 맞춰서 자동으로 실행해준다.
<br>
유니티의 특별한 함수(이벤트함수 또는 메시지함수라고 부른다)는 아래와 같은 것들이 있다.<br>
Start, Update, OnMouseDown, OnCollisionEnter 등등.. 유니티를 사용하면서 이미 겪어봤을 것이다.

### 3) static
- 클래스 안에 있는 변수와 함수를 각각 필드와 메서드라고 부른다.<br> 
필드와 메서드를 사용하려면 먼저 그 클래스에 해당하는 객체를 생성해야 한다. <br>
...
<br><br>
추가) 유니티에서 코드로 객체를 생성하지 않았고, 필드와 메서드 앞에 static을 붙이지 않았는데도 사용이 가능한 것은 스크립트를 Scene에 있는 오브젝트에 추가하면 유니티가 객체를 생성해주기 때문이다.
```csharp
using UnityEngine;
...
public class Test : MonoBehaviour
{
    ...
    static void func(){
        a +=1; // 여기서 오류가 발생한다.
        // 변수 a는 static이 아니기 때문에 객체가 없는 상황에서는 쓸 수 없다.         
    }
}
