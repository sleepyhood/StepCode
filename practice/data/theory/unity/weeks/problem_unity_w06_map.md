...
|---|---|---|---:|---|
| X01 | 변형 | GetAxis + Translate 이동 루틴 | 22 | 가상 축(Axis) 입력 데이터 읽기, 프레임 독립적인 deltaTime 스칼라 보정, 그리고 위치 갱신(Translate) 적용이라는 이동 표준 루틴 구성 능력 자체를 코드로 전이해 훈련 |
| X02 | 함정 | GetKey/Down/Up 타이밍 구분 | 38 | 다소 직관적이지 않은 `GetKeyUp`(해제 순간 1프레임) 등의 영어 단어 착각 오개념을 사전 제거하고 유지/순간 이벤트의 타이밍 한계를 검증 |
