...
|---|---|---|---:|---|
| X01 | 변형 | 타입-함수 매칭 코드화 | 32 | Float 파라미터와 SetFloat API의 매칭을 단순 암기가 아닌 런타임 블렌드 트리 동기화 실전 코딩으로 전이시켜 검증 |
| X02 | 함정 | Trigger API 선택 | 35, 32 | Trigger가 Bool/Int/Float과 본질적으로 다른 1회성 신호(펄스) 방식임을 인지하고, SetTrigger만이 유일한 올바른 호출임을 확정짓는 오개념 제거 |
