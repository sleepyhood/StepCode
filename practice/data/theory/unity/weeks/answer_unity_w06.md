...
| 문항 ID | 정답 | 한 줄 근거 |
|---|---|---|
| P01 | (1) 키 누름 유지: `Input.GetKey(KeyCode.LeftArrow)` / (2) 최초 눌림 발생: `Input.GetKeyDown(KeyCode.LeftArrow)` / (3) 키 해제(뗌): `Input.GetKeyUp(KeyCode.LeftArrow)` | 입력 상태(유지/최초 눌림/해제)별 전용 API의 정확한 역할 구분 |
| P02 | C | 주어진 유니티 함수 후보들 중 객체의 실제 로컬 좌표 위치를 이동(Translate)시켜주는 것은 `Translate(Vector3)` 메서드뿐임 |
| X01 | B | XZ축 입력 데이터 수집 -> 벡터화 -> deltaTime 곱연산을 통한 프레임 보정의 정석 이동 루틴 선택 |
...
- 판별 기준: "누르고 있는 동안(연속적 프레임)"인지, 아니면 "단발적으로 최초 발생한 그 프레임 한 번"인지 문제에서 요구하는 시점 타이밍 빈도를 문맥에서 먼저 구분해냅니다.
...
### X01
- 개념 정의: 3D 캐릭터가 땅 위를 가장 매끄럽게 걷게 하려면, `Input.GetAxis` API를 통해 상하/좌우 방향키의 가중치(-1.0 ~ 1.0)를 받아와 X와 Z축의 `Vector3` 방향 이동 변수에 담은 후, 프레임 레이트에 구애받지 않도록 보정 계수인 `Time.deltaTime`과 오브젝트 속도 `speed`를 모두 함께 스칼라 곱연산으로 곱해주어야 합니다.
- 오답 포인트: Y축 위치에 입력 값을 잘못 대입하여 캐릭터가 하늘로 솟아오르게 하거나(A, C), `Time.deltaTime` 보정을 이동 연산에서 누락시키거나(A), 단순히 `position`에 대입해 매 순간 순간이동하게 만드는 등의 함정이 있습니다(D).
- 판별 기준: `Transform.Translate` 이동 함수의 파라미터가 축 방향 벡터(`move`) X 프레임 보정(`Time.deltaTime`) X 속력 상수(`speed`) 의 3요소를 완벽히 포함했는지, 그리고 축 방향도 X와 Z로 바르게 매핑되었는지(Y축은 0) 확인하여 B를 선택합니다.
