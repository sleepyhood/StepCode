...
|---|---|---|---:|---|
| X01 | 변형 | childCount/GetChild 배열 패턴 | 14 | C# 코딩 및 실무에서 자주 쓰이는 자식 탐색 순회 패턴(`childCount` + `GetChild`)의 독립적 구현 능력을 점검 |
| X02 | 함정 | Awake vs OnEnable 역할 분리 | 26 | 단 1번 호출해야 할 초기화(`Awake/Start`) 시점과, 오브젝트가 활성화될 때마다 호출되는 이벤트(`OnEnable`)의 생명주기 기능 차이를 완벽히 구별하도록 훈련 |
