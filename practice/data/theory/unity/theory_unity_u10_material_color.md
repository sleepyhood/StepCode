...
![머티리얼 컴포넌트 체인](./data/theory/images/unity_u10_material_color_renderer.svg)
...
- 개념: RGB 값을 담고 있는 구조체가 `Color` 클래스이다. 유니티는 `Color.red`, `Color.white` 처럼 자주 쓰는 원색을 미리 만들어 두었다. (사용자 정의는 `new Color(R, G, B, A)` 형식으로 1.0f 비율로 섞음). 또한 셰이더 버전(빌트인 파이프라인 vs URP)에 따라 메인 컬러 변수의 내부 닉네임 문자열이 옛날엔 `"_Color"`, 현재는 `"_BaseColor"`로 다르므로, 안전하게 바꾸기 위해 `material.SetColor("속성명", 변경할_Color값)` 함수를 주로 쓴다.
- 오답 포인트: 파란색을 칠하려 할 때 텍스트(`"blue"`)를 넣거나 0~255 스케일의 단순 정수(예: `255, 0, 0`)를 유니티의 `new Color()` 안에 우겨넣어 에러를 터뜨리는 경우이다.
- 정답 판별: 미리 정의된 색상 상수는 `Color.xxx` 형식을 지켰는지 판단하며, 최신 파이프라인(URP) 호환을 위해 접근 키워드가 올바르게 입력됐는지 식별한다.
