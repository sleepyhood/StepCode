...
class OpenDataLoaderGUI:
    def __init__(self, root):
        ...
        # --- [1. 입력 파일 설정 영역] ---
        frame_input = tk.LabelFrame(root, text="1. 대상 PDF 파일 선택 (일괄 변환 권장)", padx=10, pady=10)
        ...
        tk.Button(btn_frame, text="+ PDF 파일 추가", command=self.add_files).pack(side=tk.LEFT, padx=2)
        tk.Button(btn_frame, text="+ 폴더 단위 추가", command=self.add_folder).pack(side=tk.LEFT, padx=2)
        tk.Button(btn_frame, text="- 선택 삭제", command=self.remove_selected).pack(side=tk.LEFT, padx=2)
        tk.Button(btn_frame, text="초기화", command=self.clear_files).pack(side=tk.RIGHT, padx=2)

        # --- [2. 출력 폴더 설정 영역] ---
        frame_output = tk.LabelFrame(root, text="2. 결과물 저장 폴더", padx=10, pady=10)
        frame_output.pack(fill=tk.X, padx=15, pady=5)

        tk.Entry(frame_output, textvariable=self.output_dir, state='readonly', width=50).pack(side=tk.LEFT, padx=5, fill=tk.X, expand=True)
        tk.Button(frame_output, text="경로 지정", command=self.select_output_dir).pack(side=tk.RIGHT)
        ...
        tk.Checkbutton(frame_format, text="Markdown (.md)", variable=self.var_md).pack(side=tk.LEFT, padx=10)
        tk.Checkbutton(frame_format, text="JSON (.json)", variable=self.var_json).pack(side=tk.LEFT, padx=10)

        # --- [4. 실행 및 로그 영역] ---
        self.btn_run = tk.Button(
            root, text="일괄 추출 시작 (Batch Convert)", command=self.start_conversion,
            bg="#2196F3", fg="white", font=("Arial", 11, "bold"), pady=10
        )
        self.btn_run.pack(fill=tk.X, padx=15, pady=15)

        tk.Label(root, text="작업 로그:").pack(anchor=tk.W, padx=15)
        self.log_area = scrolledtext.ScrolledText(root, height=8, bg="#f4f4f4", state=tk.DISABLED)
    ...
    def start_conversion(self):
        ...
        formats = []
        if self.var_md.get(): formats.append("markdown")
        ...
        # UI 비활성화 (중복 실행 방지)
        self.btn_run.config(state=tk.DISABLED, text="변환 중... (시간이 걸릴 수 있습니다)")
        self.log("=== 추출 작업 시작 ===")
        self.log(f"총 {len(self.input_files)}개 파일 Batch 처리 중...")

        # 무거운 변환 작업은 별도의 스레드에서 실행하여 UI 멈춤 방지
        threading.Thread(target=self.run_conversion_task, args=(self.input_files.copy(), self.output_dir.get(), format_str), daemon=True).start()
