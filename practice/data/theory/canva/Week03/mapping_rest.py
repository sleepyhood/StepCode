...
md_path = r"C:\Users\osw\Desktop\Workspace\#Projects\StepCode\practice\data\theory\canva\Week03\03_1차검수\3차시. 디자인 정돈 및 AI 창작 워크플로 체험.md"
pdf_txt_path = r"C:\Users\osw\Desktop\Workspace\#Projects\StepCode\practice\data\theory\canva\Week03\pdf_text.txt"
img_dir = r"C:\Users\osw\Desktop\Workspace\#Projects\StepCode\practice\data\theory\canva\Week03\images"
...
ph_matches = list(re.finditer(r'\[이미지 삽입: (IMG_W03_[0-9]{3})\]', md_content))
...
# The placeholders before 043 were generated correctly.
# 044 onwards starts from physical page 199 (as marked: <!-- p.199 --> inside the markdown? Let's check: Yes, line 489
current_pdf_page = 199 - 154
result = {}
...
for m in ph_matches:
    ...
    phys_page = current_pdf_page + 154
...
# Read existing 03_mapping.json
with open(r'C:\Users\osw\Desktop\Workspace\#Projects\StepCode\practice\data\theory\canva\Week03\03_mapping.json', 'r', encoding='utf-8') as f:
    existing = json.load(f)

existing.update(result)

# replace inside the markdown
for k, v in existing.items():
    if v and v['file_name']:
        img_str = f"![{v['alt_text']}](../images/{v['file_name']})"
        md_content = md_content.replace(f"[이미지 삽입: {k}]", img_str)
        md_content = md_content.replace(f"[이미지 매칭 실패: {k}]", img_str)

with open(r'C:\Users\osw\Desktop\Workspace\#Projects\StepCode\practice\data\theory\canva\Week03\03_1차검수\3차시. 디자인 정돈 및 AI 창작 워크플로 체험.md', 'w', encoding='utf-8') as f:
    f.write(md_content)

print(f"Replaced {len(existing)} tags.")
