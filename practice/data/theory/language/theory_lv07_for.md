...
여러 입력에서 최댓값과 최솟값을 동시에 찾는 패턴입니다.

여기서는 입력 범위를 `-1000 ~ 1000`으로 가정합니다.  
그래서 초기값을 범위 밖으로 둡니다.

- `maxv = -1001` (모든 입력보다 작음)
- `minv = 1001` (모든 입력보다 큼)

이렇게 두면 첫 입력에서 `maxv`, `minv`가 모두 확실히 갱신됩니다.  
입력 범위를 모를 때는 첫 입력값으로 초기화하는 방식이 더 안전합니다.
