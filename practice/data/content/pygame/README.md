...
각 모듈 폴더는 웹 카드, lesson, interactive, worksheet가 하나의 묶음으로 연결됩니다.
...
특히 모듈 09와 모듈 10은 기본 과정의 연장선 위에 있는 심화 응용입니다.

- 모듈 09: 좌표 변화, 각도, 값 해석이 누적된 뒤 다루는 궤적 이동 모듈
- 모듈 10: 새 기능 추가보다 긴 코드를 구조화해 읽는 마지막 모듈

즉, `7~10`은 확장 구간이지만
그중 `9~10`은 권장 선수지식이 있는 심화 응용으로 해석합니다.
...
즉, `주차`라는 표현은 과거 제작 문맥이나 내부 경로 설명에서만 제한적으로 남고,
...
특히 `round01`은 더 이상 `모듈 01 전체`를 뜻하지 않습니다.
현재 기준에서는 아래처럼 해석합니다.
...
즉, 기존의 `1주차 1~6번` 해석은 현재 운영 기준으로 사용하지 않습니다.
