...
- 5.1, 5.2, 5.3을 바탕으로 회전 중심과 회전 속도 변화를 함께 판단하게 한다.

문제:

5.2와 5.3을 반영한 상태라고 가정한다.
그 상태에서 제시 코드의 `BLOCK_A_START`와 `BLOCK_A_END` 사이를
아래 코드로 바꾸시오.
...
- 각도 증가량이 `3`에서 `1`로 줄어 회전 속도가 느려진다.
- 중심 보정이 적용되어 화면 가운데를 기준으로 도는 것처럼 보인다.
- 배경색은 같은 값으로 유지된다.
...
- 중심 보정과 각도 증가량 변경을 함께 적용하면 회전 속도가 느려지고 중심 회전이 보인다.

검수 체크:

- GIF 참고 자료와 문항 설명이 같은 동작을 가리키는지 확인
...
- 6.2와 6.3을 바탕으로 입력 규칙과 출력 스타일 변화를 함께 판단하게 한다.

문제:

6.2와 6.3을 반영한 상태라고 가정한다.
그 상태에서 제시 코드의 `BLOCK_A_START`와 `BLOCK_A_END` 사이를
아래 코드로 바꾸시오.
...
- 클릭 조건이 들어가므로 움직이기만 해서는 점이 찍히지 않는다.
- 색상은 검정에서 빨간색 계열로, 반지름은 `5`에서 `8`로 바뀐다.
...
- 클릭한 상태에서만 빨간 점이 더 크게 그려진다.

검수 체크:

- GIF 참고 자료와 문항 설명이 같은 출력 규칙을 가리키는지 확인

---

## 총평

현재 6대문항은 모듈 01 핵심 축에 맞춰 재구성되었다.
