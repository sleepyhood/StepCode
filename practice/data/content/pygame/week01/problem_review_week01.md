...
- 종료 처리 블록의 핵심 구성요소를 정확히 고르게 한다.

문제:

2.1에서 확인한 원인을 고치기 위한
종료 처리 블록 설명으로 옳은 것을 모두 고른 것은?

- ㄱ. `pygame.event.get()`으로 이벤트를 읽어야 한다.
- ㄴ. `if event.type == QUIT:` 조건이 필요하다.
- ㄷ. 종료 시 `pygame.quit()`와 `sys.exit()`를 함께 호출할 수 있다.
- ㄹ. `pygame.draw.circle()`이 종료 처리의 핵심이다.
...
- 종료 처리의 핵심은 이벤트를 읽고, `QUIT`를 확인한 뒤, 종료 함수를 호출하는 것이다.
...
사각형 인수 `(140, 90, 170, 130)`의 의미로 알맞은 것을 고른 것은?

- ㄱ. `x좌표, y좌표, 가로길이, 세로길이`
- ㄴ. `가로길이, 세로길이, x좌표, y좌표`
- ㄷ. `중심 x좌표, 중심 y좌표, 반지름, 테두리 두께`
- ㄹ. `왼쪽 위 x좌표, 오른쪽 아래 y좌표, 색상값, 두께`
...
제시된 코드에서 더 앞에 보이는 도형을 고른 것은?

- ㄱ. 사각형
- ㄴ. 원
- ㄷ. 배경
- ㄹ. 두 도형이 완전히 같은 앞뒤 순서로 보인다
