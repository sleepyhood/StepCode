...
def spawn_meteor():
    ...
    # 4차시까지는 고정된 크기(40)였습니다.
    meteor_size = 40
    
    # TODO_확장1: [주문] "운석 크기가 20에서 80 사이로 랜덤하게 나오게 바꿔주세요!"
    # 힌트: random.randint(20, 80)을 활용하여 위의 변수를 수정해보세요.
...
def check_collision():
    ...
    for meteor in meteors:
        if ship_rect.colliderect(meteor):
            # 4차시까지는 부딪히면 무조건 게임 오버였습니다.
            # game_over = True
            
            # TODO_확장2: [주문] "바로 죽지 않고, 목숨(lives)이 1 깎이게 바꿔주세요."
            # 그리고 "연속으로 맞지 않게, 부딪힌 뒤 meteors 리스트를 비워(clear)주세요"
            # 마지막으로 "목숨이 0 이하가 되면 그때 game_over = True가 되게 해주세요!"
            pass
...
def check_restart(keys):
    ...
    if keys[pygame.K_r]:
        ...
        # TODO_확장2 수정 부가작업: 재시작 시 lives를 다시 3으로 채워주세요.
        pass
