...
## 9. 인터랙티브 스테이지 잠금 (Stage-Gating) 구현 가이드

이 코스는 단순히 넘겨서 보는 슬라이드를 넘어, 핵심 목표의 끝자락에서 **정해진 문제를 풀어야만 다음 파트로 넘어갈 수 있는 스테이지 잠금 시스템**을 지원한다. 이를 템플릿으로 다른 레슨에도 자유롭게 확장 적용할 수 있다.

### 9.1. 스테이지 분리 문법 (`_stage`)
슬라이드의 논리적 진행도를 그루핑(Grouping)하려면 슬라이드 헤더 주석에 `_stage` 변수를 명시한다.
한 번 `_stage: N`이 선언되면, 다른 `_stage` 주석이 등장할 때까지 이후의 모든 슬라이드는 뷰어 하단의 **노드형 프로그레스 맵**에서 해당 번호의 그룹으로 자동 묶여 상속된다.

```md
<!-- _stage: 1 -->
# 1.1. 첫 번째 주제
(이후 슬라이드는 모두 스테이지 1로 취급)
...
### 9.2. 잠금 슬라이드 및 퀴즈 생성 (`_locked`)
특정 슬라이드에서 학생의 진행을 막고 미션을 완수하게 하려면 `_locked: true` 주석과 `.theory-mini-check-card` HTML 구조를 조합한다.

- `data-answer`: 정답 텍스트 (채점 시 대소문자 및 띄어쓰기는 파서가 알아서 무시 처리해 줌).
- `.stage-key-input`: 학생이 정답을 타이핑할 인풋 폼.
- `.stage-unlock-btn`: 검증 및 잠금 해제 제출 버튼.

```html
<!-- _stage: 2 -->
<!-- _locked: true -->

# [Mission] 다음 코드를 채워보세요.

<div class="theory-mini-check-card" data-answer="ship_rect.x += 5">
  <input type="text" class="stage-key-input" placeholder="정답 입력...">
  <button class="stage-unlock-btn">제출하고 다음으로</button>
</div>
```

### 9.3. 설계 주의사항 및 렌더링 원리
- `_locked: true`가 걸린 슬라이드에 진입하는 순간부터 하단의 네비게이션 프로그레스 노드는 `🔒` 락 아이콘으로 변하며 `[다음]` 통행이 보안 처리된다.
- 퀴즈를 풀어 정답이 일치하면 엔진(`theory.js`)이 해당 슬라이드의 잠금 상태를 메모리 상에서 영구적으로 해제(`false`) 처리하고 다음 단계로 자동 리다이렉트시킨다. (재진입 시 다시 풀 필요 없음)
- 미션(퀴즈)이 나타나는 슬라이드는 학생의 주의력이 집중되어야 하므로 가급적 좌우 2단 레이아웃이나 과도한 시각 자료 배치를 **엄격히 지양**한다. 오직 단일 코드 힌트와 퀴즈 카드 폼만 중앙에 도드라지게 배치하는 것을 표준 사례로 삼는다.
